...
include("../Connection/Connection.php");
    ...
    if (isset($_GET["action"])) {
        ...
        $sqlQry = "SELECT * from tbl_tool t inner join tbl_toolcategory tc on tc.toolcat_id =t.toolcat_id  inner join tbl_shop s on s.shop_id=t.shop_id inner join tbl_place p on s.place_id=p.place_id inner join tbl_district di on p.district_id=di.district_id  where true ";
       
        if ($_GET["category"]!=null) {

            $category = $_GET["category"];

            $sqlQry = $sqlQry." AND tc.toolcat_id IN(".$category.")";
        }
		...
		if ($_GET["name"]!=null) {
            ...
            $sqlQry = $sqlQry." AND t.tool_name LIKE '".$name."%'";
        }
...
<div class="col-md-3 mb-2">
                            <div class="card-deck">
                                <div class="card border-secondary">
                                    <img src="../Assets/Files/shoplogo/<?php echo $rowS["tool_img"]; ?>" class="card-img-top" height="250">
                                    ...
                                    <div class="card-body">
                                        <h4 class="card-title text-danger" align="center">
                                            Price : <?php echo $rowS["tool_rate"]; ?>/-
                                        </h4>
                                        <p align="center">
                                            <?php echo $rowS["toolcat_name"]; ?><br>
                                        </p>
                                        <?php
											// $average_rating = 0;
											// $total_review = 0;
										...
										//    }
										   ?>
                                            ...
                                            $stock = "select sum(toolstock_quantity) as stock from tbl_toolstock where tool_id = '" . $rowS["tool_id"] . "'";
											$result2 = $con->query($stock);
                            				$row2=$result2->fetch_assoc();
											
											$stocka = "select sum(cart_quantity) as stock from tbl_cart where tool_id = '" . $rowS["tool_id"] . "'";
